...
public class DiscountsServiceImpl implements IDiscountsService {
    ...
    public DiscountsVO add(DiscountsVO discountsVO) {
        if (discountsVO == null) {
            throw new SavaException(("插入用户失败,出入实体为空"));
        }
        ...
        Gson gson = new Gson();
        logsService.createLog("【添加】出入信息", gson.toJson(discountsVO), 1, 1);
        if (insert > 0) {
            return discountsVO;
        }
    ...
    public DiscountsPO save(DiscountsVO discountsVO) {
        if (discountsVO == null) {
            throw new CommonException(501, "出入实体为空");
        }
        ...
        throw new SavaException("出入修改失败");
    }
    ...
    public Integer deleteById(String id) {
        ...
        Gson gson = new Gson();
        logsService.createLog("【删除】出入信息", gson.toJson(check), 1, 1);
    ...
    public DiscountsVO saveone(DiscountsVO discountsVO) {
        ...
        Gson gson = new Gson();
        logsService.createLog("【修改】出入信息", gson.toJson(before) + "@*@" + gson.toJson(save), 1, 1);
        return discountsVO;
    }
